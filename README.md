...
Проект покрыт модульными тестами для обоих компонентов — backend и bot. Тесты используют `pytest` и `pytest-asyncio`, моки вместо реальной БД и внешних сервисов, поэтому запускаются локально без Docker и PostgreSQL.

### Структура тестов

```
backend/tests/
├── conftest.py          # Общие фикстуры (мок сессии БД, тестовые env-переменные)
├── test_security.py     # Хеширование паролей, создание/декодирование JWT-токенов
├── test_schemas.py      # Валидация Pydantic-схем (User, Habit, HabitLog)
└── test_services.py     # Бизнес-логика (регистрация, auth, CRUD привычек, статистика)

bot/tests/
├── conftest.py          # Общие фикстуры (мок Response, очистка token_storage)
├── test_api_services.py # API-клиент и сервисы (login, refresh, CRUD, tracking)
├── test_keyboards.py    # Построение inline-клавиатур (меню, привычки, поля)
└── test_storage.py      # Хранилище JWT-токенов (TokenStorage)
```
...
- Создание и декодирование access/refresh JWT-токенов
- Валидация входных данных Pydantic-схемами (граничные значения, обязательные поля)
- Регистрация пользователя, проверка уникальности Telegram ID
- Аутентификация (успех, неверный пароль, несуществующий пользователь)
- Создание привычки с конвертацией `reminder_time` в UTC по часовому поясу
- Отметка выполнения: новый лог, смена статуса, защита от отрицательного прогресса
- Расчёт статистики (процент выполнения, статус «выполнено сегодня»)
- Перенос невыполненных привычек и деактивация достигших цели
- Частичное обновление привычки

**Bot (49 тестов):**
- Сохранение, получение и удаление токенов в `TokenStorage`
- Регистрация и вход через API (успех/неудача, сохранение токенов)
- Обновление access-токена (успех, отсутствие токенов, очистка при неудаче)
- Авторизованные запросы (добавление Bearer-заголовка, повторный запрос после 401)
- CRUD привычек и отметка выполнения через API-клиент
- Статистика по привычке
- Построение всех inline-клавиатур (callback-данные, префиксы, структура кнопок)

### Установка зависимостей

Зависимости для тестов уже включены в `backend/pyproject.toml`
(группа `dev`): `pytest`, `pytest-asyncio`, `httpx`.
...
```bash
cd backend && pytest tests/ -v && cd ../bot && pytest tests/ -v
```

### Настройка окружения для тестов

Тестовые переменные окружения задаются автоматически в `conftest.py`
каждого компонента (через `os.environ.setdefault`), поэтому отдельный
файл `.env` для запуска тестов не требуется:

- **Backend:** `POSTGRES_*`, `SECRET_KEY` и параметры JWT
- **Bot:** `TELEGRAM_TOKEN`, `API_BASE_URL`, `REQUEST_TIMEOUT`

Тесты не обращаются к реальной базе данных и внешним API — вся
работа с БД и HTTP замокана через `unittest.mock` (AsyncMock, MagicMock).
